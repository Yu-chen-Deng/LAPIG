...
if __name__ == "__main__":
    ...
    # %% K=20 setups
    dataset_root = fullfile("./data/CMP")
    ...
    ckpt_file = "./_Compensation/checkpoint/blender_pretrained_CompenNeSt_l1+ssim_50000_32_20000_0.0015_0.8_2000_0.0001_20000.pth"
    ...
    for data_name in data_list:
        ...
        (
            ...
            prj_valid,
            mask_corners,
        ) = loadData(dataset_root, data_name, input_size, CompenNeSt_only=False)
        ...
        for num_train in num_train_list:
            ...
            for model_name in model_list:
                ...
                for loss in loss_list:
                    ...
                    with open(
                        join("./data/CMP", f"{surface}_affine_mat.pkl"), "wb"
                    ) as f:
                        pickle.dump(affine_mat, f)
                    warping_net.set_affine(affine_mat.flatten())
                    if torch.cuda.device_count() >= 1:
                        warping_net = nn.DataParallel(
                            warping_net, device_ids=device_ids
                        ).to(device)
                    ...
                    if torch.cuda.device_count() >= 1:
                        compen_nest_pp = nn.DataParallel(
                            compen_nest_pp, device_ids=device_ids
                        ).to(device)
                    ...
                    # train model
                    compen_nest_pp, valid_psnr, valid_rmse, valid_ssim = trainModel(
                        compen_nest_pp, train_data, valid_data, train_option
                    )

                    # uncompensated metrics
                    cam_raw_valid = readImgsMT(fullfile(data_root, "cam/raw/test"))
                    cam_desire_valid = readImgsMT(fullfile("./data/CMP", "desire/test"))
                    ...
                    if save_compensation:
                        ...
                        desire_test_path = "./data/CMP/desire/test"
                        ...
                        with torch.no_grad():
                            # simplify CompenNet++
                            compen_nest_pp.module.simplify(
                            ...
                            prj_cmp_test, _, _ = compen_nest_pp(
                                desire_test, cam_surf_test
                            )  # compensated prj input image x^{*}

                            prj_cmp_test.detach()
                        del desire_test, cam_surf_test, cam_surf_texture
                        ...
                        # prj_cmp_crop_path = fullfile(data_root, "prj/cmpwowarp")
                        # if not os.path.exists(prj_cmp_path):
                        #     os.makedirs(prj_cmp_path)
                        # cam_surf_path = fullfile(data_root, "prj/surf")
                        # if not os.path.exists(cam_surf_path):
                        #     os.makedirs(cam_surf_path)

                        # save images cam_surf_test
                        saveImgs(
                            prj_cmp_test, prj_cmp_path, 10000
