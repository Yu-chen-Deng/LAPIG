...
def loadData(
    ...
    if not CompenNeSt_only:
        # find projector FOV mask（下标1 投影纯白色 下标2 投影纯黑色）
...
def initCompenNeSt(compen_nest, dataset_root, device):
    ckpt_file = "./_Compensation/checkpoint/init_CompenNeSt_l1+ssim_500_24_2000_0.001_0.2_800_0.0001.pth"
...
def trainModel(net, train_data, valid_data, train_option):
    ...
    checkpoint_file_name = fullfile(checkpoint_dir, f"{title}_compen.pth")
