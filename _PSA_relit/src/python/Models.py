...
class PSA_relit_PCNet(nn.Module):
    ...
    def __init__(self, mask=torch.ones((1, 3, 256, 256)), shading_net=None, fix_shading_net=False, use_mask=True, use_rough=True):
...
class PSA_relit_PCNet_clamp(nn.Module):
    # Project-and-Capture Network for SPAA
    def __init__(self, mask=torch.ones((1, 3, 256, 256)), clamp_min=0, clamp_max=1, shading_net=None, fix_shading_net=False, use_mask=True, use_rough=True):
        super(PSA_relit_PCNet_clamp, self).__init__()
        self.name      = self.__class__.__name__
        self.use_mask  = use_mask
        self.use_rough = use_rough
        ...
        # initialize from existing models or create new models
        # self.warping_net = copy.deepcopy(warping_net.module) if warping_net is not None else WarpingNet()
        self.shading_net = copy.deepcopy(shading_net.module) if shading_net is not None else ShadingNetSPAA_clamp()
...
class PSA_relit_CompenNeStPlusplus_clamp(nn.Module):
    def __init__(self, warping_net=None, clamp_min=0, clamp_max=1, compen_net=None):
        ...
        # initialize from existing models or create new models
        # self.warping_net = copy.deepcopy(warping_net.module) if warping_net is not None else WarpingNet()
        self.compen_nest = copy.deepcopy(compen_net.module) if compen_net is not None else CompenNeSt_clamp()
