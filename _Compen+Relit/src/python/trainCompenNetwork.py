...
def loadData(
    dataset_root, data_name, input_size, data_type="raw", CompenNeSt_only=False
):
    ...
    prj_train_path = fullfile(dataset_root, "prj", "train")
    prj_valid_path = fullfile(dataset_root, "prj", "test")

    print("Loading data from '{}'".format(data_root))

    # training data
    cam_surf_texture = readImgsMT(cam_ref_path, index=[1])
    cam_surf = readImgsMT(cam_ref_path, index=[2])
    # gray: ref/img_0126.png is cam-captured surface image i.e., s when img_gray.png i.e., x0 projected
    ...
    if not CompenNeSt_only:
        ...
        im_diff = readImgsMT(cam_ref_path, index=[1], size=input_size) - readImgsMT(
            cam_ref_path, index=[0], size=input_size
        )
        ...
        for i in range(im_diff.shape[-1]):
            ...
            prj_fov_mask[i, :, :, :] = repeat_np(
                torch.Tensor(np.uint8(im_mask)).unsqueeze(0), 3, 0
            )
        ...
        cam_train = torch.where(prj_fov_mask, cam_train, torch.tensor([0.0]))
        cam_valid = torch.where(prj_fov_mask, cam_valid, torch.tensor([0.0]))
...
def initCompenNeSt(compen_nest, dataset_root, device):
    ckpt_file = "./_Compen+Relit/checkpoint/init_CompenNeSt_l1+ssim_500_24_2000_0.001_0.2_800_0.0001.pth"
    ...
    if os.path.exists(ckpt_file):
        ...
        init_data = dict(cam_surf=cam_surf, cam_train=prj_train, prj_train=prj_train)
...
def trainModel(net, train_data, valid_data, train_option):
    ...
    while iters < train_option["max_iters"]:
        ...
        prj_train_pred = predict(
            net, dict(cam=cam_train_batch, cam_surf=cam_surf_train_batch)
        )
        ...
        if train_option["pre-trained"]:
            ...
            if iters <= 200:
                train_loss_batch, train_l2_loss_batch = computeLoss(
                    prj_train_pred, prj_train_batch, "l1"
                )
            else:
                train_loss_batch, train_l2_loss_batch = computeLoss(
                    prj_train_pred, prj_train_batch, train_option["loss"]
                )
        else:
            train_loss_batch, train_l2_loss_batch = computeLoss(
                prj_train_pred, prj_train_batch, train_option["loss"]
        ...
        if valid_data is not None and (
            ...
            valid_psnr, valid_rmse, valid_ssim, prj_valid_pred = evaluate(
                net, valid_data
            )
    ...
    checkpoint_dir = "./data/Compen+Relit"
    if not os.path.exists(checkpoint_dir):
        os.makedirs(checkpoint_dir)
    checkpoint_file_name = fullfile(
        checkpoint_dir,
        train_option["data_name"],
        f"{title}_{train_option['model_name']}.pth",
    )
...
def predict(net, data):
    ...
    if type(prj_pred) == tuple and len(prj_pred) > 1:
        prj_pred = prj_pred[0]
    return prj_pred
    ...
    with torch.no_grad():
        ...
        if cam_surf.device.type != device.type:
            ...
            for i in range(0, num_valid // batch_size):
                ...
                # predict batch
                prj_valid_pred_batch = predict(
                    net, dict(cam=cam_valid_batch, cam_surf=cam_surf_batch)
                ).detach()
                if (
                    type(prj_valid_pred_batch) == tuple
                    and len(prj_valid_pred_batch) > 1
                ):
                    prj_valid_pred_batch = prj_valid_pred_batch[0]
                prj_valid_pred[last_loc : last_loc + batch_size, :, :, :] = (
                    prj_valid_pred_batch.cpu()
                )

                # compute loss
                valid_mse += (
                    l2_fun(prj_valid_pred_batch, prj_valid_batch).item() * batch_size
                )
            ...
            prj_valid_pred = predict(
                net, dict(cam=cam_valid, cam_surf=cam_surf)
            ).detach()
